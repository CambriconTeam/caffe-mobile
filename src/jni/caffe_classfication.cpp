...
CaffeClassification *CaffeClassification::caffe_classification_ = NULL;

CaffeClassification *CaffeClassification::get() {
  return caffe_classification_;
}

CaffeClassification *CaffeClassification::get(const string &param_file,
                              const string &trained_file) {
  if (!caffe_classification_) {
    try {
      caffe_classification_= new CaffeClassification(param_file, trained_file);
...
CaffeClassification::CaffeClassification(const string &param_file, const string &trained_file) {
  // Load Caffe model
...
bool CaffeClassification::predictImage(const uint8_t* rgba,
                               int channels,
